...
import AddCustomItemModal from "./AddCustomItemModal"; // ✅ Import your custom item modal
...
interface AddOrderModalProps {
  ...
  formData: {
    ...
    organization: string;
    phone?: string;
    items: { inventoryId: string; itemName: string; quantity: number; isCustom?: boolean }[];
  };
  ...
  onAddCustomItem: (name: string, quantity: number, category?: string) => void;
...
const AddOrderModal: React.FC<AddOrderModalProps> = ({
  ...
  const handleAddCustomItem = (name: string, quantity: number, category?: string) => {
    onAddCustomItem(`${name} (Custom)`, quantity, category);
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100]">
        <div className="bg-white dark:bg-background-100 rounded-2xl shadow-2xl w-full max-w-3xl border border-background-200 dark:border-background-300 max-h-[90vh] overflow-y-auto m-4">
          ...
          <form onSubmit={onSubmit} className="p-6 space-y-6">
            ...
            {/* Customer fields */}
            <div>
              <label className="block text-sm font-medium text-text-900 dark:text-white mb-2">Customer Name <span className="text-red-500">*</span></label>
              <input
                type="text"
                name="customer_name"
            ...
            <div>
              <label className="block text-sm font-medium text-text-900 dark:text-white mb-2">Email <span className="text-red-500">*</span></label>
              <input
                type="email"
                name="email"
            ...
            <div>
              <label className="block text-sm font-medium text-text-900 dark:text-white mb-2">Organization <span className="text-red-500">*</span></label>
              <input
                type="text"
                name="organization"
            ...
            <div>
              <label className="block text-sm font-medium text-text-900 dark:text-white mb-2">Phone Number</label>
              <input
                type="tel"
                name="phone"
            ...
            {/* Select Items */}
            <div>
              <label className="block text-sm font-medium text-text-900 dark:text-white mb-2">Select Item</label>

              {formData.items.map((row, idx) => {
                const options = getAvailableOptions(idx);
                const itemInfo = inventoryItems.find((i) => i._id === row.inventoryId);
                // allow custom items to render
                const quantity = row.quantity;
                const maxQuantity = itemInfo?.quantity || quantity;
                ...
                return (
                  <div key={row.inventoryId} className="flex items-center gap-3 mb-3">
                    <select
                      ...
                      disabled={isSubmitting || row.inventoryId.startsWith("custom-")} // disable selection for custom items
                    >
                      {row.inventoryId.startsWith("custom-") ? (
                        <option value={row.inventoryId}>{row.itemName}</option>
                      ) : (
                        <>
            ...
            {/* Footer */}
            <div className="flex justify-between items-center mt-6 pt-6 border-t border-background-200 dark:border-background-300">
              {/* Left: Custom Item Button */}
              <div className="text-sm text-text-700 dark:text-text-300">
                Don’t see the item you want?{" "}
                <button
                  type="button"
                  onClick={() => setShowCustomItemForm(true)}
                  className="text-primary-900 dark:text-primary-500 underline font-medium hover:text-primary-800 dark:hover:text-primary-400 transition-colors"
                >
                  Add Custom Item
                </button>
              </div>
